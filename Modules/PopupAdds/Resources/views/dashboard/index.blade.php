...
@section('title', __('popup_adds::dashboard.popup_adds.routes.index'))
@section('content')
        ...
        <div class="page-content">
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">{{__('popup_adds::dashboard.popup_adds.routes.index')}}</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        ...
                        @permission('add_popup_adds')
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.popup_adds.create')) }}" class="btn sbold green">
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.image')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.status')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.type')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.start_at')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.end_at')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.created_at')}}</th>
                                    <th>{{__('popup_adds::dashboard.popup_adds.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.popup_adds.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.popup_adds.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
...
responsivePriority:1,
                            width: '13%',
                            title: '{{__('popup_adds::dashboard.popup_adds.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.popup_adds.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.popup_adds.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                return `
                                    @permission('edit_popup_adds')
                                        <a href="` + editUrl + `" class="btn btn-sm blue" title="Edit">
                                          <i class="fa fa-edit"></i>
                                        </a>
                                    @endpermission

                                    @permission('delete_popup_adds')
                                    @csrf
