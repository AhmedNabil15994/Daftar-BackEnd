...
@section('seo')
    @include('meta::manager', [
        'title'         => $category->translate(locale())->title,
        'keywords'      => $category->translate(locale())->seo_keywords,
        'description'   => $category->translate(locale())->seo_description,
...
<div class="container">
    ...
    <div class="library-cat-block">
        <div class="owl-carousel library_categories">
            ...
            @foreach ($categories as $categoryBanner)
                <div class="item">
                    <a href="{{ url(route('frontend.categories.index',[
                $vendor->translate(locale())->slug,
                $categoryBanner->translate(locale())->slug
                ])) }}">
                        <img src="{{url($categoryBanner->image)}}" alt=""/>
                        <h6>{{ $categoryBanner->translate(locale())->title }}</h6>
                    </a>
                </div>
            @endforeach
        </div>
    ...
    <div class="inner-page row">
        ...
        <div class="col-md-3">
            <div class="content-sidebar">
                <form action="{{url(route('frontend.categories.filters.index',[
                $vendor->translate(locale())->slug,
                                            ...
                                            <div class="checkboxes one-in-row">
                                                ...
                                                @foreach ($category->children as $categoryFilter)
                                                    <input id="check-{{$categoryFilter->id}}" type="checkbox"
                                                           name="categories_filter[]" value="{{ $categoryFilter->id }}">
