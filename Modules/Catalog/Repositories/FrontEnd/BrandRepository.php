...
class BrandRepository
{
    ...
    public function getAllActiveByCategory($category, $order = 'id', $sort = 'desc')
    {
        $brands = $this->brand->active();
        if (!is_null($category)) {
            $brands = $brands->whereHas('products.categories', function ($query) use ($category) {
                $query->where('categories.id', $category->id);
            });
        }
        return $brands->orderBy($order, $sort)->get();
    }
