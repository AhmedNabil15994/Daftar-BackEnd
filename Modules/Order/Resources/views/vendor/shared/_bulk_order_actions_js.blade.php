<script>
    function onBulkActionsChange(printPage = 'orders') {
        ...
        if (value === 'edit_delivery_status') {
            var status = $('#deliveryStatusSelect').val();
            if (status !== '') {
                applyBulkActions('{{ url(route('vendor.orders.update_bulk_delivery_status')) }}',
            ...
            printAllChecked('{{ url(route('vendor.orders.print_selected_items')) }}', printPage);
        } else if (value === 'delete') {
            {{--  applyBulkActions('{{ url(route('vendor.orders.deletes')) }}', 'delete');  --}}
        } else {
    ...
    function returnMessageType(type) {
        ...
        switch (type) {
            case 'edit_delivery_status':
                text = '{{ __('apps::dashboard.general.bulkDeliveryStatus_message') }}';
                break;
            case 'delete':
    ...
    $(document).ready(function() {
        $("#bulkActionsSelect").change(function() {
            if ($(this).val() === 'edit_delivery_status') {
                $('#deliveryStatusSelect').show();
            } else {
                $('#deliveryStatusSelect').hide();
            }
        });
    });
    ...
    function printAllChecked(url, page = '') {
        ...
        if (ids != null && ids !== '' && ids.length > 0) {
            ...
            alert("{{ __('apps::dashboard.general.select_at_least_one_item') }}");
        }
    }
</script>
