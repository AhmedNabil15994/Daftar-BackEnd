        ...
        <div class="row">
            <div class="col-md-12">
                <div class="portlet light bordered">
                    ...
                    @permission('add_times')
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.times.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('deliverytime::dashboard.times.datatable.from')}}</th>
                                    <th>{{__('deliverytime::dashboard.times.datatable.to')}}</th>
                                    <th>{{__('deliverytime::dashboard.times.datatable.status')}}</th>
                                    <th>{{__('deliverytime::dashboard.times.datatable.created_at')}}</th>
                                    <th>{{__('deliverytime::dashboard.times.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('dashboard.times.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.times.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
...
responsivePriority:1,
              width: '13%',
              title: '{{__('deliverytime::dashboard.times.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.times.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.times.destroy", ":id") }}';
      					deleteUrl = deleteUrl.replace(':id', data);

      					return `
                @permission('edit_times')
      						<a href="`+editUrl+`" class="btn btn-sm blue" title="Edit">
      			              <i class="fa fa-edit"></i>
      			            </a>
      					@endpermission

                @permission('delete_times')
                @csrf
