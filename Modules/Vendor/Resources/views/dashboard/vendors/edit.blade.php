                                            ...
                                            <ul class="nav nav-pills nav-stacked">
                                                ...
                                                <li>
                                                    <a href="#rating" data-toggle="tab">
                                                        {{ __('vendor::dashboard.vendors.update.form.rating') }}
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                        ...
                        <div class="col-md-9">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane fade in" id="other">
                                    ...
                                    <div class="col-md-10">
                                                    ...
                                                    @foreach ($payments as $payment)
                                                        <label class="mt-checkbox">
                                                            <input type="checkbox" name="payment_id[]"
                                                                   value="{{$payment->id}}" {{ $vendor->payments->contains($payment->id) ? 'checked=""' : ''}}>
                                                            <img src="{{ url($payment->image) }}" alt=""
                                                                 style="width: 26px;">
                                                            {{ __('transaction::frontend.transactions.payments.'.$payment->code) }}
                                                            <span></span>
                                                        </label>
                                                    @endforeach
                                                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-9">
                                                <select name="section_id" id="select2-single-append"
                                                        class="form-control select2-allow-clear">
                                                    <option value=""></option>
                                                    @foreach ($sections as $section)
                                                        <option
                                                            value="{{ $section['id'] }}" {{ $vendor->sections->contains($section->id) ? 'selected=""' : ''}}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-9">
                                                <select name="seller_id[]" id="single" class="form-control select2"
                                                        multiple>
                                                    <option value=""></option>
                                                    @foreach ($sellers as $seller)
                                                        <option
                                                            value="{{ $seller['id'] }}" {{ $vendor->sellers->contains($seller->id) ? 'selected=""' : ''}}>
                                                            {{ $seller['name'] }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                ...
                                <div class="tab-pane fade in" id="rating">
                                    <h3 class="page-title">{{__('vendor::dashboard.vendors.update.form.rating')}}</h3>
                                    <div class="col-sm-3 col-xs-6 no-print">
                                        <div class="ticket-counter">
                                            <h2 class="label label-warning bold uppercase">
                                                {{ number_format($vendor->rating()->avg('rate'),1) }} / 5
                                            </h2>
                                        </div>
                                        <hr>
                                        @foreach ($vendor->rating as $rate)
                                            <ul>
                                                <li>{{ $rate->rate }} / 5</li>
                                                <li>#{{ $rate->order_id }}</li>
                                                <li>{{ date('d-m-Y' , strtotime($rate->created_at)) }}</li>
                                            </ul>
                                            @if (!$loop->last)
                                                <hr>
                                            @endif
